...
function loadFighter(
  ...
  new GLTFLoader().load(
    ...
    (gltf) => {
      ...
      if (gltf.animations.length > 0) {
        mixer = new THREE.AnimationMixer(model);
        // "RifleAimingIdle" is a real motion-captured idle (retargeted from
        // a Mixamo clip, no gun attached or held — just the pose/animation
        // itself) — this is the one meant to actually ship now, in place
        // of the older "Idle2" breathing idle.
        const idleClip = gltf.animations.find((c) => c.name === "RifleAimingIdle") ?? gltf.animations.find((c) => c.name === "Idle2") ?? gltf.animations.find((c) => c.name.includes("Idle")) ?? gltf.animations[0];
